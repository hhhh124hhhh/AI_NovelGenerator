...
class MainWorkspace(ctk.CTkFrame):
    ...
    def _create_content_area(self):
        ...
        content_frame.grid(row=2, column=0, sticky="nsew", padx=10, pady=(5, 10))
        content_frame.grid_columnconfigure(0, weight=2)  # 章节编辑器占据更多空间
        content_frame.grid_columnconfigure(1, weight=1)  # 日志面板占据次要空间
        content_frame.grid_rowconfigure(0, weight=1)

        # 左侧：章节编辑器
        self._create_chapter_editor(content_frame)

        # 右侧：日志面板
        self._create_log_panel(content_frame)

    def _create_chapter_editor(self, parent):
        """创建章节内容编辑器"""
        # 章节编辑器容器
        editor_frame = ctk.CTkFrame(parent, corner_radius=6)
        editor_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5), pady=5)
    ...
    def _create_log_panel(self, parent):
        """创建日志输出面板"""
        # 日志面板容器
        log_frame = ctk.CTkFrame(parent, corner_radius=6)
        log_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 0), pady=5)
        log_frame.grid_rowconfigure(1, weight=1)
        log_frame.grid_columnconfigure(0, weight=1)

        # 日志面板标题栏
        log_header_frame = ctk.CTkFrame(log_frame, corner_radius=6, fg_color="transparent")
    ...
    def _initialize_log_output(self):
        """初始化日志输出"""
        try:
            # 添加初始日志信息
            initial_log = """=== AI小说生成器 - 输出日志 ===
系统已就绪，等待开始生成...
...
"""
            self._append_log(initial_log)
        except Exception as e:
            logger.error(f"初始化日志输出失败: {e}")

    def _append_log(self, message):
        """追加日志消息（线程安全）"""
        try:
            if hasattr(self, 'log_output'):
                self.after(0, lambda: self._safe_append_log(message))
        except Exception as e:
    ...
    def log_message(self, message, level="INFO"):
        ...
        try:
            ...
            self._append_log(formatted_message)
        except Exception as e:
            logger.error(f"记录日志消息失败: {e}")

    def _create_generation_controls(self):
        """创建生成操作区域 - 包含主生成按钮和步骤按钮"""
        # 生成控制容器
        generation_frame = ctk.CTkFrame(self.main_frame, corner_radius=8)
        generation_frame.grid(row=1, column=0, sticky="ew", padx=10, pady=(5, 10))
        generation_frame.grid_columnconfigure(0, weight=1)
        generation_frame.grid_rowconfigure(0, weight=0)  # 主生成按钮行
        generation_frame.grid_rowconfigure(1, weight=0)  # 步骤按钮行

        # 第一行：主生成按钮
        self._create_main_generate_button(generation_frame)

        # 第二行：步骤按钮
        self._create_step_buttons(generation_frame)

    def _create_main_generate_button(self, parent):
        """创建主生成按钮 - 大型、醒目"""
        main_generate_frame = ctk.CTkFrame(parent, corner_radius=6, fg_color="transparent")
        main_generate_frame.grid(row=0, column=0, sticky="ew", pady=(10, 5))
        main_generate_frame.grid_columnconfigure(0, weight=1)

        # 主生成按钮
        self.main_generate_button = ctk.CTkButton(
            main_generate_frame,
            text="🚀 开始生成小说",
            command=self._on_main_generate,
            font=ctk.CTkFont(size=16, weight="bold"),
            height=50,
            fg_color="#2E7D32",
            hover_color="#388E3C"
        )
        self.main_generate_button.grid(row=0, column=0, sticky="ew", padx=20)

        # 生成说明
        generate_hint = ctk.CTkLabel(
            main_generate_frame,
            text="配置参数后点击开始，或使用下方步骤按钮进行精细控制",
            font=ctk.CTkFont(size=11),
            text_color="gray"
        )
        generate_hint.grid(row=1, column=0, pady=(2, 5))

    def _create_step_buttons(self, parent):
        """创建步骤控制按钮"""
        # 按钮容器
        step_frame = ctk.CTkFrame(parent, corner_radius=8)
        step_frame.grid(row=1, column=0, sticky="ew", pady=(5, 10))
        step_frame.grid_columnconfigure((0, 1, 2, 3, 4, 5), weight=1)

        # 步骤按钮定义
        steps = [
            ("step1", "🏗️ 架构", self._on_generate_architecture),
            ("step2", "📋 目录", self._on_generate_blueprint),
            ("step3", "✍️ 草稿", self._on_generate_chapter),
            ("step4", "✨ 完善", self._on_finalize_chapter),
            ("consistency", "🔍 检测", self._on_consistency_check),
            ("batch", "🚀 批量", self._on_batch_generate)
        ]
    ...
    def _on_main_generate(self):
        """主生成按钮点击事件 - 完整生成流程"""
        try:
            if self.generation_state['is_generating']:
                messagebox.showwarning("提示", "正在生成中，请等待当前任务完成")
                return
            ...
            # 开始完整生成流程
            self._start_full_generation_process()

        except Exception as e:
            logger.error(f"主生成流程启动失败: {e}")
    ...
    def _start_full_generation_process(self):
        """启动完整生成流程"""
        try:
            # 更新UI状态
            self.generation_state['is_generating'] = True
            self.main_generate_button.configure(
                text="⏸️ 生成中...",
                state="disabled",
                fg_color="#757575"
            )
            ...
            if hasattr(self, 'status_label'):
                self.status_label.configure(text="🔄 开始生成流程...")

            # 启动生成线程
            import threading
            generation_thread = threading.Thread(
                target=self._full_generation_worker,
                daemon=True
            )
    ...
    def _full_generation_worker(self):
        """完整生成流程工作线程"""
        try:
            # 生成步骤序列
            steps = [
                ("架构生成", self._on_generate_architecture),
                ("目录生成", self._on_generate_blueprint),
                ("章节生成", self._on_generate_chapter)
            ]

            for step_name, step_func in steps:
                # 更新状态
                self._update_generation_status(f"正在进行{step_name}...")
            ...
            # 生成完成
            self._update_generation_status("✅ 生成流程完成！")

        except Exception as e:
            logger.error(f"生成流程执行失败: {e}")
            self._update_generation_status(f"❌ 生成失败: {str(e)}")
        finally:
            # 重置状态
            self._reset_generation_state()

    def _update_generation_status(self, message):
        """更新生成状态（线程安全）"""
        try:
    ...
    def _create_compact_params(self):
        ...
        params_frame.grid(row=0, column=0, sticky="ew", padx=10, pady=(5, 10))
